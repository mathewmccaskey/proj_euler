...
c-------------------------------------------------------------------------------------------------c
c                                                                                                 c
c  This function returns a gaussian random number given the mean and standard deviation.          c
c                                                                                                 c
c-------------------------------------------------------------------------------------------------c
      implicit none
...
c parameters for this subroutine only
      ...
      double precision fac, gset, rsq      
      save iset, gset
      data iset/0/

      if (iset.eq.0) then
 1      x1 = 2.d0*random(idum)-1.d0
        x2 = 2.d0*random(idum)-1.d0
        rsq = x1**2 + x2**2
        if ((rsq .ge. 1.d0).or.(rsq .eq. 0.d0)) goto 1
        fac = dsqrt(-2.d0*dlog(rsq)/rsq)
        gset = sd*x1*fac + mean
        randgauss = sd*x2*fac + mean
        iset = 1
      else
        randgauss = gset
        iset = 0
...
c-------------------------------------------------------------------------------------------------c
      function randgauss2(idum,mean,sd)
c-------------------------------------------------------------------------------------------------c
c                                                                                                 c
c  Supposedly this is another way of getting gaussian variables.  If the check works then it      c
c  shouldn't matter which function to use.                                                        c
c                                                                                                 c
c-------------------------------------------------------------------------------------------------c
      implicit none
...
c parameters for this subroutine only
      integer iset
      double precision random, randgauss2, x1, x2
      ...
      if (iset .eq. 0) then
        x1 = random(idum)
        x2 = random(idum)
        randgauss2 = mean + sd*dsqrt(-2.d0*dlog(x1))*dcos(2.d0*pi*x2)
        gset = mean + sd*dsqrt(-2.d0*dlog(x1))*dsin(2.d0*pi*x2)
        iset = 1
      else
        randgauss2 = gset
        iset = 0
